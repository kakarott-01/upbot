...
export const BotControlsModals = forwardRef<BotControlsModalsRef, Props>(function BotControlsModals(
  { confirmStart, confirmMarketStop, handleStopAll }, ref
) {
    ...
    <>
      ...
      {stopModal && (
        <MarketStopModal
          ...
          onDrain={() => { confirmMarketStop(stopModal.market, 'graceful'); setStopModal(null) }}
      ...
      {showStopAllModal && (
        <StopAllModal
          ...
          hasLiveMarkets={(modeData?.markets ?? []).some((m: any) => m.mode === 'live' && (botData?.activeMarkets ?? []).includes(m.marketType))}
